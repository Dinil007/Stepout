...
Applied to cleaned world positions to reduce noise from tracking artifacts.
Supports Moving Average, Savitzky-Golay, and Kalman filtering.
Falls back to Savitzky-Golay by default.
"""
...
class TrajectorySmoother:
    """
    Smooths player trajectories using configurable methods.
    """
    ...
    def __init__(
        ...
        """
        Initializes TrajectorySmoother.

        Args:
            method: Smoothing method - 'moving_avg', 'savgol', or 'kalman'.
            window_size: Window size for Moving Average and Savitzky-Golay.
            polyorder: Polynomial order for Savitzky-Golay.
            kalman_process_noise: Process noise covariance for Kalman filter.
            kalman_measurement_noise: Measurement noise covariance for Kalman.
        """
        self.method = method
    ...
    def _kalman(self, positions: np.ndarray) -> np.ndarray:
        """Applies 2D Kalman filter smoothing."""
        try:
    ...
    def smooth_trajectory(
        ...
        """
        Smooths a single player's trajectory.

        Args:
            cleaned_points: List of dicts with 'clean_world_position'.

        Returns:
            List of dicts with added 'smoothed_world_position'.
        """
    ...
    def smooth_tracks_batch(
        self,
        cleaned_tracks: Dict[int, List[Dict]]
    ) -> Dict[int, List[Dict]]:
        """
        Smooths all player trajectories.

        Args:
            cleaned_tracks: Dict mapping track_id to cleaned trajectory points.

        Returns:
            Dict mapping track_id to smoothed trajectory points.
        """
        smoothed_tracks = {}
        for track_id, points in cleaned_tracks.items():
            smoothed_tracks[track_id] = self.smooth_trajectory(points)
            logger.info("Track %d smoothed with %d points", track_id, len(points))
