...
Cleans and prepares ball tracking data by removing duplicates, large jumps,
missing positions, and short tracks. Interpolates short gaps.
"""
...
class BallTrajectoryCleaner:
    """
    Cleans raw ball trajectory data and interpolates short gaps.
    """
    ...
    def __init__(
        ...
        duplicate_threshold_m: float = 0.05
    ):
        """
        Initializes cleaner.

        Args:
            max_jump_m: Maximum allowed displacement between consecutive frames.
            min_track_frames: Minimum frames for valid track.
            max_gap_frames: Max consecutive missing frames to interpolate.
            duplicate_threshold_m: Distance below which positions are duplicates.
        """
    ...
    def clean_trajectory(
        ...
        """
        Cleans a single ball trajectory.

        Args:
            track_data: List of dicts with track_id, frame_number, timestamp,
                       world_position, confidence.

        Returns:
            Cleaned list with 'clean_world_position', or None if invalid.
        """
        ...
        for i, point in enumerate(track_data):
            ...
            dist = np.sqrt((pos[0] - last_valid['pos'][0])**2 + (pos[1] - last_valid['pos'][1])**2)
    ...
    def clean_batch(
        ...
        """
        Cleans all ball trajectories.

        Args:
            all_tracks: Dict mapping track_id to raw track data.
        ...
        for track_id, track_data in all_tracks.items():
            ...
            if result is None:
                ...
                logger.info("Ball track %d rejected: too short", track_id)
            else:
                cleaned_tracks[track_id] = result
                issues['total_valid_frames'] += len(result)
                issues['total_interpolated_frames'] += sum(1 for p in result if p.get('interpolated', False))

        issues['rejection_rate'] = issues['rejected_too_short'] / len(all_tracks) if all_tracks else 0.0
